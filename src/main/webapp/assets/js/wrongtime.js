...
function getwrongoverexamtypes(){
	var appenddata ="";
	...
	$.ajax({
		...
		success:function(data){
			for(var i=0; i < data.length; i++){
				appenddata += '<div class="col-md-2  setresp" ><a href="'+base_url+"jsp/student/livezone/wrong&timetest.jsp?exam_id="+data[i].id+'"><img src="'+data[i].image+'" width="80px" height="80px"><p class="heit">'+data[i].name+'</p></a></div>';
				}
			 $("#wrongexamtypes").empty();
			 $("#wrongexamtypes").append(appenddata);
	...
	$.ajax({
		...
		success:function(data){
			...
			for(var i=0; i < data.length; i++){
				...
				appenddata += '<li style="text-align:center;"><a href="#" onclick ="wrngchapetrsbyid(this);" style="box-shadow: 0px 0px 18px grey" sub-id = "'+data[i].id+'" exam-id="'+examidid+'" role="tab" data-toggle="tab"><img src="'+data[i].image+'" width="50px" height="50px"><p class="heit1">'+data[i].name+'</p></a></li>';
				}
			 $("#settabs").empty();
...
function wrngchapetrsbyid(elem){
	 var appenddata = "";
var LoginData = getLoginData();
	 var subjid = $(elem).attr('sub-id');
	 var examid = $(elem).attr('exam-id');
		var inputData = {
		"subjectId" : subjid,
	...
	$.ajax({
		...
		dataType : "json",
		contentType : 'application/json',
		url : base_url + 'rest/student/getWrongOverTimeQuestions',
		data : inputdata,
	    beforeSend: function(){
	    	$("#chapters").empty();
	    	$(".load").html('<img src="'+base_url+'assets/images/students/spinner2.gif" width="100" height="100"  >');			
	    	 $("#chapters").append('<p class="text-center"style="margin-top:10px">Please Wait For Your Personalized Practice....'+'</p>');
			...
			var wrongintime = "Wrong (WithInTime)";
			
			var overtimcor = "Overtime (Correct)";
			
			var overtimewrg = "Overtime (Wrong)";
			
			for (var i = 0; i < data.length; i++) {
			     var chapid = data[i].chapterId;
			     if(data[i].issubscribed)
					{
			          ...
			          if(data[i].wrongWithInTime == 0 && data[i].overTimeWrong == 0 && data[i].overTimeCorrect == 0 && data[i].finished == 0){
			        	  appenddata += '';
			          }else{
			    	 appenddata += '<div class="row testzone_chapterwise_tests '+mask+'" style="margin-bottom:25px;box-shadow: 0px 0px 18px grey;"><div class="row">'+
						'<div class="col-lg-8 col-md-8 col-sm-8 col-xs-6"><span>'+
						'<img src="'+data[i].chapterIcon+'" width="40px" height="40px" style="border-radius:25px"></span>&nbsp;&nbsp;'+
								'<span class="test_name_previous_years" style="position:relative;top:-15px;">'+data[i].chapterName+'</span></div>'+
							'<div class="col-lg-4 col-md-4 col-sm-4 col-xs-6">'+
								'<span style="float:right;position:relative;top:-2px;left:-5px;color:black;" >No of tests attended by you:<span style="font-weight:bold;font-size:17px;color:#6731C1;margin-left:4px;">'+data[i].noOfTestAttended+'</span></span>'+
							'</div></div><div class="row">'+
							'<div class="col-md-12 col-md-offset-1">'+
							'<div class="col-md-3"><p style="color:black;">Wrong&nbsp;(WithInTime)</p>'
							if(data[i].wrongWithInTime == 0){
								global = global + data[i].wrongWithInTime;
								appenddata += '<p><a href="#"><span style="color:#C16CB9;font-size:17px;font-weight:bold;position:relative;left:40px;">'+data[i].wrongWithInTime+'</span></a></p>'
							}else{
								global = global + data[i].wrongWithInTime;
								appenddata += '<p><a href="'+base_url+'jsp/student/livezone/wrongtimetestscreen.jsp?examid='+examid+'&questype='+wrongintime+'&subjid='+subjid+'&chpid='+chapid+'" ><span style="color:#C16CB9;font-size:17px;font-weight:bold;position:relative;left:40px;">'+data[i].wrongWithInTime+'</span></a></p>'
							}
			    	 appenddata += '</div><div class="col-md-3"><p style="color:black;">Overtime&nbsp;(Wrong)</p>'
			    	 if(data[i].overTimeWrong == 0){
			    		 global = global + data[i].overTimeWrong;
			    		 appenddata += '<p><a href="#"><span style="color:#DB5A7C;font-size:17px;font-weight:bold;position:relative;left:40px;">'+data[i].overTimeWrong+'</span></a></p>'
			    	 }else{
			    		 global = global + data[i].overTimeWrong;
			    		 appenddata += '<p><a href="'+base_url+'jsp/student/livezone/wrongtimetestscreen.jsp?examid='+examid+'&questype='+overtimewrg+'&subjid='+subjid+'&chpid='+chapid+'"><span style="color:#DB5A7C;font-size:17px;font-weight:bold;position:relative;left:40px;">'+data[i].overTimeWrong+'</span></a></p>'
			    	 }
			    	 appenddata += '</div><div class="col-md-3"><p style="color:black;">Overtime&nbsp;(Correct)</p>'
				           if(data[i].overTimeCorrect == 0){
				        	   global = global + data[i].overTimeCorrect;
				        	   appenddata += '<p><a href="#" ><span style="color:#00BA85;font-size:17px;font-weight:bold;position:relative;left:40px;">'+data[i].overTimeCorrect+'</span></a></p>'
				           }else{
				        	   global = global + data[i].overTimeCorrect;
				        	   appenddata += '<p><a href="'+base_url+'jsp/student/livezone/wrongtimetestscreen.jsp?examid='+examid+'&questype='+overtimcor+'&subjid='+subjid+'&chpid='+chapid+'" ><span style="color:#00BA85;font-size:17px;font-weight:bold;position:relative;left:40px;">'+data[i].overTimeCorrect+'</span></a></p>'

				           }
			    	 global = global + data[i].finished;
			    	 appenddata += '</div><div class="col-md-3"><p style="color:black;">Finished</p><p style="color:green;cursor:pointer;"><span data-id='+data[i].chapterId+' onclick="showCorrectanswer(this)"><a  data-toggle="modal" data-target="#correctansmodal" style="color:#51A842;font-size:17px;font-weight:bold;position:relative;left:20px;">'+data[i].finished+'</a></span></p></div></div></div></div>';
			      }
...
function Getwrongtimetestscreen(){
	var questionid = "";
	var comprehensionid= "";
   ...
   var questiontype = getParameterByName("questype");
	...
	$.ajax({
		...
		dataType : "json",
		contentType : 'application/json',
		url : base_url + 'rest/student/getWrongOverTimeExamPaper',
		...
		success : function(data) {
			...
			appendsubjects += "<span><a aria-expanded='true' style='cursor:pointer;font-weight:bold;font-size:16px;' >"+questiontype+"</a></span>";	
...
function finishtest(){
	...
	$.ajax({
		type : "POST",
		url : base_url + 'rest/student/submitWrongAndOverTimeQuestions',
		dataType : "json",
		contentType : 'application/json',
		data:stringfy,
		 beforeSend: function(){
...
function showCorrectanswer(elem){
	...
	$.ajax({
		type : "POST",
		url : base_url + 'rest/student/getWrongAndOverTimeFinishedQuestions',
		dataType : "json",
		contentType : 'application/json',
		data:strcorrect,
		 beforeSend: function(){
...
function wrongoveranalytics(){
	...
	$.ajax({
		type : "POST",
		url : base_url + 'rest/student/getWrongAndOverTimeResults',
		dataType : "json",
		contentType : 'application/json',
		data:stranlay,
		 beforeSend: function(){
...
/*		         '<div class="col-md-12 text-center">'+
		         ...
		         '<p class="col-md-6 col-sm-6"style="color:gray;font-weight:bold;"><a>'+data[0].visited+'</a></p>'+
		         '</div>'+*/
		         '<div class="col-md-12 text-center">'+
		         '<p class="col-md-6 col-sm-6" style="color:gray;font-weight:bold;">Unattempted Qns.</p>'+
		         '<p class="col-md-6 col-sm-6" style="color:gray;font-weight:bold;" ><a >'+data[0].unAttempted+'</a></p>'+
		         '</div>'+
		         '<div class="col-md-12 text-center">'+
		         '<p class="col-md-6 col-sm-6" style="color:green;font-weight:bold;">Correct Qns.</p>'+
		         '<p class="col-md-6 col-sm-6" style="color:green;font-weight:bold;"><a>'+data[0].correct+'</a></p>'+
		         '</div>'+
		         '<div class="col-md-12 text-center">'+
		         '<p class="col-md-6 col-sm-6" style="color:red;font-weight:bold;">Wrong</p>'+
		         '<p class="col-md-6 col-sm-6" style="color:red;font-weight:bold;" ><a>'+data[0].wrong+'</a></p>'+
		         '</div>'+
		         '</div>'+
		         '<hr>'+
		         '<div class="row">'+
