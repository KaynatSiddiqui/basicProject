...
function seventycornerexamtypes(){
	var appenddata ="";
	...
	$.ajax({
		...
		success:function(data){
			for(var i=0; i < data.length; i++){
				appenddata += '<div class="col-md-2"><a href="'+base_url+"jsp/student/seventycorner/seventypercentsubjects.jsp?exam_id="+data[i].id+'"><img src="'+data[i].image+'" width="80px" height="80px"><p class="heit">'+data[i].name+'</p></a></div>';
				}
			 $("#seventyexamtypes").empty();
			 $("#seventyexamtypes").append(appenddata);
...
function seventysubjectbyexamid(){
	var appenddata ="";
	...
	$.ajax({
		type:"GET",
		url : base_url+'rest/student/getWrongOverTimeSubjects/'+examidid,
		data:"json",
		contentType:'application/json',
		success:function(data){
			var nthchild = '';
			...
			for(var i=0; i < data.length; i++){
				...
				appenddata += '<li style="text-align:center;"><a href="#" onclick ="subjcount(this)" style="box-shadow: 0px 0px 18px grey" sub-id="'+data[i].id+'" exam-id="'+examidid+'" role="tab" data-toggle="tab"><img src="'+data[i].image+'" width="50px" height="50px"><p class="heit1">'+data[i].name+'</p></a></li>';
				}
				
			 $("#settabs").empty();
			 $("#settabs").append(appenddata);
			 $("#settabs li:nth-child("+parseInt(nthchild)+") a").trigger("click");
		
			 
		},
	   error:function(data){
		   $("#emptysubj").modal("show");
		   $(".loadershow").hide();
...
function subjcount(elem){
	
	var subid = $(elem).attr('sub-id');
	
	var exmid = $(elem).attr('exam-id');
	...
	var appendata = "";
	var substr = JSON.stringify(subdetails);
	
	var global = 0;
	
	$.ajax({
		type:"POST",
		url : base_url+'rest/student/getSeventyPercentageChapters',
		dataType:"json",
		contentType:'application/json',
		data:substr,
	    beforeSend: function(){
	    	...
	    	$(".load").show();	
			$("#subjdetails").append('<p class="text-center"style="margin-top:10px">Please Wait For Your Personalized Practice...'+'</p>');
		...
		success:function(data){
			...
			var chaplist = data.chaptersLists;
			
			for(var i = 0;i < chaplist.length;i++){
				
				var chapid = chaplist[i].chapterId;
				...
				if(chaplist[i].unAttemptedQuestionsCount == 0 && chaplist[i].wronglyAnsweredCount == 0 && chaplist[i].correctlyAnsweredCount == 0 && chaplist[i].finished == 0){
					appendata += " ";
				}else{
					appendata += '<div class="row testzone_chapterwise_tests '+mask+'" style="margin-bottom:25px;box-shadow: 0px 0px 18px grey">'+
			'<div class="row">'+
				'<div class="col-lg-8 col-md-8 col-sm-8 col-xs-6">'+
				   '<span><img src='+chaplist[i].chapterIcon+' width="40px" height="40px" style="border-radius:25px"></span>&nbsp;&nbsp;'+
					'<span class="test_name_previous_years" style="position:relative;top:-15px;">'+chaplist[i].chapterName+'</span>'+
				'</div>'+
				'<div class="col-lg-4 col-md-4 col-sm-4 col-xs-6">'+
					'<span style="float:right;position:relative;top:-2px;left:-5px;color:black;" >No of tests attempted by you:<span style="font-weight:bold;font-size:17px;color:#6731C1;margin-left:4px;">'+chaplist[i].numOfTestsAttended+'</span></span>'+
				'</div>'+
			'</div>'+
			'<div class="row">'+
				'<div class="col-md-12 col-md-offset-1">'+
				'<div class="col-md-3">'+
				'<p style="color:black;">70% - Unattempted</p>'
				if(chaplist[i].unAttemptedQuestionsCount == 0){
					global = global + chaplist[i].unAttemptedQuestionsCount;
					appendata += '<p><a href="#"><span style="color:#C16CB9;font-size:17px;font-weight:bold;position:relative;left:30px;">'+chaplist[i].unAttemptedQuestionsCount+'</span></a></p>'
				}else{
					global = global + chaplist[i].unAttemptedQuestionsCount;
					appendata += '<p><a href="'+base_url+'jsp/student/seventycorner/seventytestscreen.jsp?examid='+exmid+'&questype='+unattempted+'&subjid='+subid+'&chpid='+chapid+'" ><span style="color:#C16CB9;font-size:17px;font-weight:bold;position:relative;left:30px;">'+chaplist[i].unAttemptedQuestionsCount+'</span></a></p>'
				}
					appendata += '</div><div class="col-md-3"><p style="color:black;">70% - Wrong</p>'
			 if(chaplist[i].wronglyAnsweredCount == 0){
				 global = global + chaplist[i].wronglyAnsweredCount;
				 appendata += '<p><a href="#"><span style="color:#DB5A7C;font-size:17px;font-weight:bold;position:relative;left:30px;">'+chaplist[i].wronglyAnsweredCount+'</span></a></p>'
			 }else{
				 global = global + chaplist[i].wronglyAnsweredCount;
				 appendata += '<p><a href="'+base_url+'jsp/student/seventycorner/seventytestscreen.jsp?examid='+exmid+'&questype='+wrngcou+'&subjid='+subid+'&chpid='+chapid+'" ><span style="color:#DB5A7C;font-size:17px;font-weight:bold;position:relative;left:30px;">'+chaplist[i].wronglyAnsweredCount+'</span></a></p>'
			 }
			appendata += '</div><div class="col-md-3"><p style="color:black;">70% - Correct</p>'
				  if(chaplist[i].correctlyAnsweredCount == 0){
					  global = global + chaplist[i].correctlyAnsweredCount;
					  appendata += '<p><a href="#"><span style="color:#00BA85;font-size:17px;font-weight:bold;position:relative;left:30px;">'+chaplist[i].correctlyAnsweredCount+'</span></a></p>'
				  }else{
					  global = global + chaplist[i].correctlyAnsweredCount;
					  appendata += '<p><a href="'+base_url+'jsp/student/seventycorner/seventytestscreen.jsp?examid='+exmid+'&questype='+corrcou+'&subjid='+subid+'&chpid='+chapid+'" ><span style="color:#00BA85;font-size:17px;font-weight:bold;position:relative;left:30px;">'+chaplist[i].correctlyAnsweredCount+'</span></a></p>' 
				  }
			 global = global + chaplist[i].finished;
			appendata +='</div><div class="col-md-3"><p style="color:black;">Finished</p><p style="color:green;cursor:pointer;"><span data-id='+chapid+' onclick="showCorrectanswer(this)"><a data-toggle="modal" data-target="#correctansmodal" style="color:#51A842;font-size:17px;font-weight:bold;position:relative;left:20px;" >'+chaplist[i].finished+'</a></span></p></div></div></div></div>'
				}
			}
...
function Getseventycornertestscreen(){
	var questionid = "";
	var comprehensionid= "";
	var subjectid = getParameterByName('subjid');
	var chapterid = getParameterByName('chpid');
	var LoginData = getLoginData();
	var examid = getParameterByName('examid');
   var questiontype = getParameterByName('questype');
	...
	var strquestionpaper = JSON.stringify(questionpaper);
	...
	$.ajax({
		...
		dataType : "json",
		contentType : 'application/json',
		url : base_url + 'rest/student/getSeventyPercentageCornorExamPaper',
		data : strquestionpaper,
		success : function(data) {
			...
			appendsubjects += "<span><a aria-expanded='true' style='cursor:pointer;font-weight:bold;font-size:16px;' >70% - "+questiontype+"</a></span>";	
						...
						if(j == 0){
								...
								var removespace = questionlisttype[j].type.replace(/ /g,'');
								 ...
								 var mainquestion = questionlisttype[j].title;
									...
									var pusharray = quesarray.push(questionlisttype[j].questionId);
								    	...
								    	sidequesmenu += '<li class='+activename+' id="sidesett" style="white-space:nowrap;overflow:hidden;"><a href="#question'+questionlisttype[j].questionId+'" onclick="onactiveques(this)" id="singlequestiondiv'+questionlisttype[j].questionId+'" style="cursor:pointer;height:52px;min-width:220px;display:flex;" data-type='+removespace+' data-questionid='+questionlisttype[j].questionId+' data-comprehensionid='+questionlisttype[j].comprehensionId+'  data-toggle="tab"><div class="round round-sm hollow singlebckground'+questionlisttype[j].questionId+'">'+(j+1)+'</div>&nbsp;&nbsp;&nbsp;&nbsp;<span>'+mainquestion+'<span></a></li>';			
							...
							if(questionlisttype[j].type == 'Comprehension'){
		                           ...
		                           var mainquestion = questionlisttype[j].title;
									...
									sidequesmenu += '<li class='+activename+' id="sidesett" style="white-space:nowrap;overflow:hidden;"><a href="#question'+questionlisttype[j].comprehensionId+'" onclick="onactiveques(this)" id="comprequestiondiv'+questionlisttype[j].comprehensionId+'" style="cursor:pointer;height:52px;min-width:220px;display:flex;" data-type='+questionlisttype[j].type+' data-questionid='+questionlisttype[j].questionId+' data-comprehensionid='+questionlisttype[j].comprehensionId+'  data-toggle="tab"><div class="round round-sm hollow comprebckground'+questionlisttype[j].comprehensionId+'">'+(j+1)+'</div>&nbsp;&nbsp;&nbsp;&nbsp;<span>'+mainquestion+'<span></a></li>';
							...
							if(questionlisttype[j].type == 'Multi Comprehension'){
							 ...
							 if(questionlisttype[j].type == 'Integer'){
									...
									sidequesmenu += '<li class='+activename+' id="sidesett" style="white-space:nowrap;overflow:hidden;"><a href="#question'+questionlisttype[j].questionId+'" onclick="onactiveques(this)" id="intequestiondiv'+questionlisttype[j].questionId+'" style="cursor:pointer;height:52px;min-width:220px;display:flex;" data-type='+questionlisttype[j].type+' data-questionid='+questionlisttype[j].questionId+' data-comprehensionid='+questionlisttype[j].comprehensionId+' data-toggle="tab"><div class="round round-sm hollow intebckground'+questionlisttype[j].questionId+'">'+(j+1)+'</div>&nbsp;&nbsp;&nbsp;&nbsp;<span>'+mainquestion+'<span></a></li>';
						      ...
						      if(questionlisttype[j].type == 'Matrix'){
		 							...
		 							sidequesmenu += '<li class='+activename+' id="sidesett" style="white-space:nowrap;overflow:hidden;"><a href="#question'+questionlisttype[j].questionId+'" onclick="onactiveques(this)" id="matrixquestiondiv'+questionlisttype[j].questionId+'" style="cursor:pointer;height:52px;min-width:220px;display:flex;" data-type='+questionlisttype[j].type+'  data-questionid='+questionlisttype[j].questionId+' data-comprehensionid='+questionlisttype[j].comprehensionId+' data-toggle="tab"><div class="round round-sm hollow matrixbckground'+questionlisttype[j].questionId+'">'+(j+1)+'</div>&nbsp;&nbsp;&nbsp;&nbsp;<span>'+mainquestion+'<span></a></li>';
		    ...
		    $("#wrongtimesubj").append(appendsubjects); 
		...
		error: function(data){
			alert("Failed to get questions or the chapter doesn't consists questions");
		}
...
function finishtest(){
	...
	$.ajax({
		type : "POST",
		url : base_url + 'rest/student/submitSeventyPercenatageCornor',
		dataType : "json",
		contentType : 'application/json',
		data:stringfy,
	    beforeSend: function(){
...
function showCorrectanswer(elem){
	...
	$.ajax({
		type : "POST",
		url : base_url + 'rest/student/getSeventyPercentageFinishedQuestions',
		dataType : "json",
		contentType : 'application/json',
		data:strcorrect,
		 beforeSend: function(){
...
function seventyanalytics(){

	var LoginData = getLoginData();
	var studid = LoginData.studentid;
	
	var chapid = getParameterByName('chpid');
	...
	$.ajax({
		type : "POST",
		url : base_url + 'rest/student/getSeventyPercentageAnalytics',
		dataType : "json",
		contentType : 'application/json',
		data:stranlay,
		   beforeSend: function(){
...
/*		         '<div class="col-md-12 text-center">'+
		         ...
		         '<p class="col-md-6 col-sm-6"style="color:gray;font-weight:bold;" ><a>'+data.visited+'</a></p>'+
		         '</div>'+*/
		         '<div class="col-md-12 text-center">'+
		         '<p class="col-md-6 col-sm-6" style="color:gray;font-weight:bold;">Unattempted Qns.</p>'+
		         '<p class="col-md-6 col-sm-6" style="color:gray;font-weight:bold;" ><a>'+data.unAttempted+'</a></p>'+
		         '</div>'+
		         '<div class="col-md-12 text-center">'+
		         '<p class="col-md-6 col-sm-6" style="color:green;font-weight:bold;">Correct Qns.</p>'+
		         '<p class="col-md-6 col-sm-6" style="color:green;font-weight:bold;"><a>'+data.correct+'</a></p>'+
		         '</div>'+
		         '<div class="col-md-12 text-center">'+
		         '<p class="col-md-6 col-sm-6" style="color:red;font-weight:bold;">Wrong</p>'+
		         '<p class="col-md-6 col-sm-6" style="color:red;font-weight:bold;"><a>'+data.wrong+'</a></p>'+
		         '</div>'+
		         '</div>'+
		         '<hr>'+
		         '<div class="row">'+
...
function counterli(){
	counter++;
	var selectedLI = ($("#allQuestions li.active").index()) + 1;
	counter = selectedLI;
	 
	 if(counter >= jsondata.subjectslist[currentsubjectindex].questionlist.length){
			$(".stname").html(sessionStorage.getItem("stdname"));
