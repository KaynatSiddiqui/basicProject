...
$.ajax({
	...
	for(var i=0; i<data.length; i++){
		...
		sessionStorage.setItem("examTypeIds", data[i].examTypeIds);
		sessionStorage.setItem("standardId", data[i].standardId);
		sessionStorage.setItem("mobilenumber", data[i].mobilenumber);
		sessionStorage.setItem("email", data[i].emailId);
		sessionStorage.setItem("categoryid", data[i].categoryid);
		
var emamtypes = data[i].examTypeIds;
var examnames= data[i].examTypeNames;
for(var push=0; push<emamtypes.length; push++){
	pushobject['examid_'+emamtypes[i]] = emamtypes[i];
	pushobject['examname_'+examnames[i]] = examnames[i];
}
	pushobject['student_mob'] =data[i].mobilenumber;
	pushobject['student_email'] =data[i].emailId;
	pushobject['student_name'] =data[i].name;
	pushobject['category_name']= data[i].category;
	pushobject['category_id']= data[i].categoryid;
	pushobject['standard_id']=data[i].standardId;
	pushobject['student_std_id']= data[i].id; pushobject['subscriptiontype']= 'free';
	pushobject['src'] ="landing/Registerd/Attempted";
		...
		$("#namecate").html(data[0].category);
		$("#namestan").html(data[0].standardName);
	}
...
*/
	...
	function logoutStudent(){
		sessionStorage.removeItem("examTypeIds");
		sessionStorage.removeItem("standardId");
	...
	function resizeBase64Img(imagefile, width, height) {
	  ...
	  $("<img/>").attr("src",imagefile).on('load',function() {
...
//var photo = $("#base64photo").val();
	...
	var photo = $("#compressedimage").val();
	
	var branch = $("#branch option:selected").text();
	var caste = $("#quota").val();
	var presentclass = parseInt($("#standard").val());
...
function getStudentsDetails(){
			...
			$.ajax({
				...
				if(data[0].photo !=""){
					...
					$("#branchprofile").html(data[0].branch);
					$("#casteprofile").html(data[0].caste);
					$("#presentclass").html(data[0].presentClass);
					...
					sessionStorage.setItem("catid", data[0].categoryid);
...
function attempeted(tab,examType){
	...
	var parent_id = LoginData.parentId;
		...
		$.ajax({
			...
			success: function(data){
				...
				if(data.length === 0 || data === undefined || data === null){
					...
					$("#clickmsg_"+currAttemptedTabId).html("<div style='position: relative;top: 45px;margin-right: auto;margin-left: auto;width: 316px;color:#ff55ae;font-weight:500;'>Please click on colored numbers to get questions</div>");
						...
						if(data[i].issubscribed){
						 ...
						 appenddata += "<tr id='row"+data[i].id+"'>"; 
						   ...
						   if(data[i].fromMissesd === false ){
							     appenddata += "<td class='unmask'>"+(i+1)+"</td>"; 
						   }else{
							     appenddata += "<td  class='unmask' style='text-align: center;'>"+(i+1)+"<img src='"+base_url+"assets/images/students/studentsidebar/liveMIssed.svg' class='wstedicon' data-toggle='tooltip' title='This test is missed test'></td>"; 
							   
						   }
						 if(data[i].spentLessTime === false ){
							 appenddata += "<td class='unmask'>"+data[i].testTakenDate+  "</td>"; 
						 }
						 else{
							 appenddata += "<td class='unmask'>"+data[i].testTakenDate+"<br> &nbsp; &nbsp;<a href='#'  data-id='"+data[i].examTypeId+"_"+data[i].sepntTime+"_"+data[i].totalTime+"'  data-toggle='modal' data-target='#wasteexam'  id='b1' onclick='wasteAttmpt(this);''><img src='"+imge+"' class='wstedicon' data-toggle='tooltip' title='Click To Check Your wasted Exam Time'></a>";
							 appenddata += "</td>"; 
						 }
						
						 appenddata += "<td class='unmask'>"+data[i].testName+"</td>";
						 appenddata += "<td class='unmask'>"+data[i].examType+"</td>";
						 appenddata += "<td class='unmask'>"+data[i].standard+"</td>";
						 appenddata += "<td class='unmask'>"+data[i].countryLevelRank+"</td>";
						 appenddata += "<td class='unmask'><a href='#' data-toggle='modal' data-target='#attemptedModal' data-id='"+data[i].id+"' data-whatever='"+data[i].id+"' data-subid='"+examType+"' onclick='getAttemptedScore(this,1)'><span class='text_score'>"+data[i].marksObtained+"/"+data[i].totalMarks+"<br /> ("+data[i].attempted+"Q/"+data[i].noofQuestions+"Q)</span></a></td>";
						 appenddata += "<td class="+mask+"><a class='' href='#' data-toggle='modal' data-target='#attemptedModal' data-id='"+data[i].id+"' data-whatever='"+data[i].id+"' data-subid='"+examType+"' onclick='getAttemptedScore(this,55)'><span class='text_negative_score'>"+data[i].negative+"<br />("+data[i].negativeQuestions+"Q)</span></a></td>";
						 appenddata += "<td class="+mask+"><a href='#' data-toggle='modal' data-target='#attemptedModal' data-id='"+data[i].id+"' data-whatever='"+data[i].id+"' data-subid='"+examType+"' onclick='getAttemptedScore(this,2)'><span class='text_unattempted'>"+data[i].unAttempted+"</span></a></td>";
						 appenddata += "<td class="+mask+"><a class='' href='#' data-toggle='modal' data-target='#attemptedModal' data-id='"+data[i].id+"' data-whatever='"+data[i].id+"' data-subid='"+examType+"' onclick='getAttemptedScore(this,56)'><span class='text_time_taken'>"+data[i].overtime+"</span></a></td>";
						 appenddata += "<td class="+mask+">"+data[i].accuracy+"<span>&nbsp;%</span></td>";
						 appenddata += "<td class="+mask+">"+data[i].percentage+"<span>&nbsp;%</span></td>";
						 appenddata += "<td class="+mask+">"+data[i].percentile+"</td>";
						 appenddata += "<td class="+mask+">"+data[i].speed+"</td>";
						 appenddata += "<td class="+mask+">"+data[i].topScoreMarks+"</td>";
						var  testpostid = btoa(data[i].id);
						if(parent_id !="" && parent_id !=undefined && parent_id !=null){
							appenddata += "<td  class="+mask+"><a href='"+base_url+"jsp/parent/student_analytics3.jsp?id="+testpostid+"&stid="+data[i].studentID+"&exm="+exam+"&testtype="+currTabActiveAttempted+"&examid="+data[i].examTypeId+"' class='marf btn btn-details btn-icon-anim btn-circle' data-toggle='tooltip' data-placement='left' title='Test Analytics'><i class='fa fa-line-chart font-18 mt-10'></i></a>";
							
						}else{
							appenddata += "<td  class="+mask+"><a href='"+base_url+"jsp/student/student module/student_analytics3.jsp?id="+testpostid+"&stid="+data[i].studentID+"&exm="+exam+"&testtype="+currTabActiveAttempted+"&examid="+data[i].examTypeId+"' class='marf btn btn-details btn-icon-anim btn-circle' data-toggle='tooltip' data-placement='left' title='Test Analytics'><i class='fa fa-line-chart font-18 mt-10'></i></a>";
							
						}
						  appenddata += "<p  class="+mask+" data-toggle='tooltip' data-placement='left' title='Question paper'><a  data-toggle='modal' data-target='#showmoreModal' data-whatever='@mdo' data-original-title='View Paper' class='marf btn btn-view_paper btn-icon-anim btn-circle' data-id="+data[i].id+" data-stid="+data[i].studentID+" data-subid='"+data[i].subjectID+"' onclick='attemptedshowpaperques(this)'><i class='fa fa-file-pdf font-18 mt-10'></i></a></p>";
						 appenddata += "<p  class="+mask+" data-toggle='tooltip' data-placement='left' title='Test Details'><a data-toggle='modal' data-target='#attemptedModal1' data-whatever='@mdo' data-original-title='View Paper' data-id="+data[i].id+" data-stid="+data[i].studentID+" class='btn btn-test_details btn-icon-anim btn-circle' data-subid='"+data[i].subjectID+"' onclick='Show2ndModal(this)'><i class='fa fa-th-large font-18 mt-10'></i></a></p></td></tr>";	  
			     }
					$("#attempetedbody_"+currAttemptedTabId).empty();
	...
	$.ajax({
		...
		success: function(data){
			...
			error:function(data){
				...
				$("#totalMissedCount_"+currMissedTabId).html("<div style='float:right;position: relative;left:-274px;'>Total Missed test: <span class='theme_color bold font-18' id='totalMissedCount_1'>0</span></div>");	
			}
	...
	$.ajax({
		...
		error: function(){
			...
			var nextclick = tabId+1;
				 $('#subjectTabs li:nth-child('+parseInt(nextclick)+') a').click();
				$("#questionsContent").empty();
...
function getallSelectedcities(element, event) {
	...
	if(pass != conform){
		...
		$.ajax({
			...
			success:function(data){
				  ...
				  alertify.alert(data.message,function (e) {
						if (e) {
			...
			success:function(data){
				if(data.length > 0 ){
					if(edit == "true"){
						...
						$("#occupation").val(data[0].parentoccupation);
							...
							$("#parentlogname").append(data[0].parentname);
				...
				$("#linked").append('<a href="#" onclick="linkstatus(\'delink\');" class="btn btn-danger status_btn">De-Link</a><span class="purple">(Change here)</span>');	
					...
					$("#linked").append('<a href="#" onclick="linkstatus(\'link\');" class="btn btn-primary status_btn">Link</a><span class="purple">(Change here)</span>');
					 ...
					 window.location = base_url+"jsp/student/parentlogins/parent_logins_add.jsp";
	 ...
	 var inputData ={
		...
		$.ajax({
			...
			success:function(data){
				...
				$("#linked").empty();
				if(flag == "delink"){
					alertify.alert('Hey,'+stdname+' ! <br>You have Delinked the Parent Login');
					$("#linked").append('<a href="#" onclick="linkstatus(\'link\');" class="btn btn-primary status_btn">Link</a><span class="purple">(Change here)</span>');
					
				}else{
					alertify.alert('Hey,'+stdname+' ! <br>You have Linked the Parent Login');
					$("#linked").append('<a href="#" onclick="linkstatus(\'delink\');" class="btn btn-danger status_btn">De-Link</a><span class="purple">(Change here)</span>');	
				}
 ...
 function changepwd(){
	...
	function editparent(){
		...
		var studentid  = LoginData.studentid;
		if(pphone !=""){
		if(pphone.length < 10 || pphone.length > 10){
			alert("Mobile number atleast 10 Characters");
			return false;
		}
	...
	$.ajax({
		...
		success:function(data){
			...
			for(var i=0; i < examtypes.length; i++){
                 ...
                 appenddata += '<a href="#" onclick="gettabs(\'' + examtypeid[i] + '\',\'' + previd+ '\');" id="'+examtypeid[i]+'"  class="btn btn-default btn-outline fancy-button btn-0">'+examtypes[i]+'</a>';
				}
			 $("#examtypeslist").empty();
										...
										'<div class="mt-40 livezone_toppers_header_bg p-5"><div class="row"><div class="col-sm-2">'+
											'<img class="livezone_toppers" src="'+topperimage+'"></div>'+
												'<div class="col-sm-7"><div class="livezone_student_name">'+data[i].topperName+'</div>'+
													'<div class="black">('+data[i].topperId+') </div><div class="black">'+data[i].topperState+'</div></div>'+
											...
											'<div class="row mt-20">'+
												'<div class="col-sm-6">'+
													'<span class="black">Avg Percentage % <i class="fa fa-arrow-right"> </i> '+(data[i].avgpercentage).toFixed(2)+'%</span></div>'+
												'<div class="col-sm-6">'+
													'<div class="black">Avg Accuracy % <i class="fa fa-arrow-right"> </i>  <span class="black">'+data[i].avgAccuracy+'%</span></div>'+
												'</div></div></div></div>';
	...
	$(document).ready(function(){
		...
		emiDetails();
			 ...
			 // EMI DEDAILS 
		var studentemiStatus ="";
		var sidestudentemiStatus ="";
	function emiDetails(){
		...
		$.ajax({
			type:"GET",
			url : base_url+'rest/student//EmiDetailsStatus/'+studentid,
			data:"json",
			contentType:'application/json',
			success:function(data){
				
				if(data.status){
						studentemiStatus +='<a href="#" onclick="emidetailsjsp()"><i class="fa fa-money"></i><span>Emi Details</span></a>';
						sidestudentemiStatus +='<a href="#" onclick="emidetailsjsp()"><div class="pull-left"><img id="" src="/assets/images/students/studentsidebar/WalletIcon.svg" style="position:relative;left:-6px;" width="26px"><span class="right-nav-text" id="" style="position: absolute;bottom: 4px;top:13px;margin-left: 8px">Emi Details</span></div><div class="clearfix"></div></a>';
					}
					else{
				  ...
				  $("#emiStatus").empty("");
					$('#emiStatus').append(studentemiStatus);
				$("#sideemioption").empty("");
					$('#sideemioption').append(sidestudentemiStatus);
		...
		function emidetailsjsp(){
			 window.location = base_url+"jsp/student/student module/emipayment.jsp";
		}
