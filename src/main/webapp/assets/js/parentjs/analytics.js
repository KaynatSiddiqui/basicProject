...
function optionsAddingModal(iData,resolvedAnswer,answer,resolvedSelected){
	...
	if(iData["option1"] === "" && iData["option2"] === ""){
	  ...
	  '<div class="col-lg-10 col-md-10 col-sm-10 col-xs-12 questionswrap">'+ iData.solution+
	  '</div></div></div></div>';
      ...
      '<div class="center"><div class="col-md-4"><i class="fa fa-child below-popup-icon-attempt"></i><span class="below-icon-text">  Students Attempted -</span>  <span class="below-icon-text">'+iData.totalStudentsAttended+'</span></div>'+
      '<div class="col-md-4"><i class="fa fa-check-circle below-popup-icon"></i><span class="below-icon-text"> Students Correctly Answered -</span> <span class="below-icon-text">'+iData.totalStudentsAnsweredCorrectly+'</span></div>';
      
	  if(iData.overtime){
		 appendData += '<div class="col-md-4"><i class="fa fa-hourglass-half overtime-icon"></i><span class="below-icon-text red"> Time Taken -</span> <span class="below-icon-text red"> '+iData.timespent+'</span></div>';
	  } else{
		  appendData += '<div class="col-md-4"><i class="fa fa-hourglass-half overtime-icon"></i><span class="below-icon-text"> Time Taken -</span> <span class="below-icon-text"> '+iData.timespent+'</span></div>';
	  }
 ...
 /*	var x = document.referrer;
 	...
 	*/    	
 				...
 				for(var i = 0;i<subjpercen.length;i++){
 					...
 					subjpercent += '<div class="col-sm-4"><img src='+subjpercen[i].subjectIcon+' width="30px">&nbsp;&nbsp;<span style="font-size:17px">'+subjpercen[i].subject+': <span id="subject"></span>'+subjpercen[i].percentile+'</span></div>'
...
function getSeventyCornerComprt(type){
	...
	$.ajax({
	  ...
	  error:function(data){
		 ...
		 setTimeout(function() {
			$("#popupnotcorrectanswerqns").empty();
			$("#popupnotcorrectanswerqns").html('<h5 class="center">'+headingtxt+' not found in 70 Percentage  Marks :(</h5>');
		  }, 500);
	...
	var inputData ={
		...
		$.ajax({
			...
			success:function(data){
				...
				if( best_chapters != null && best_chapters !="" && weak_chapters != undefined){
...
"testPostId": testid
		...
		$.ajax({
			...
			success:function(data){
				 ...
				 for(var i=0; i < data.length; i++){
					 ...
					 appenddata += '<tr><td >'+data[i].serialNumber+'</td><td>'+data[i].questionType+'</td><td>'+data[i].difficultyType+'</td>';
					 var qview = data[i].questionView;
					 if(qview.correct == true){
						  appenddata +='<td>Correct</td>';
					 }else{
						 appenddata +='<td>Wrong</td>';
					 }
					 
						 appenddata +=	'<td>'+data[i].numberOfStudentsCorrectlyAnswered+'</td><td><button class="btn btn-warning" data-toggle="modal" data-target="#not-correct-answer-qns" onclick="getquestioncomparitive('+i+')" style="border-radius: 25px">View</button></td></tr>';
 			...
 			var resolvedAnswer, resolvedSelected;
 				...
 				if(data.passagequestion == true && data.passagequestion !== false){
						...
						appendData += '<div class="">'+
						  ...
						  '<label class="control-label" for="example-email"><strong>Passage</strong></label>'+
						  '<span>&nbsp; : &nbsp;</span></div>'+
						  '<div class="col-lg-10 col-md-10 col-sm-9 col-xs-12 questionswrap nomar-nopad">'+data.passage +
						  '</div></div></div>';
						...
						appendData += '</div> <div class="col-lg-10 col-md-10 col-sm-9 col-xs-12 questionswrap nomar-nopad">'+data.question +
						  '</div></div></div>';
 ...
 function compoptionappend(iData,resolvedAnswer,answer,resolvedSelected)
 {
	...
	if(iData.option1 === "" && iData.option2 === ""){
	  ...
	  //Total students count for attempted and correctly answerred
	  appendData += '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mb-3 qnscount ">'+
      '<div class="center"><div class="col-md-4"><i class="fa fa-child below-popup-icon-attempt"></i><span class="below-icon-text">  Students Attempted -</span>  <span class="below-icon-text">'+iData.numberOfStudentsAttempted+'</span></div>'+
      '<div class="col-md-4"><i class="fa fa-check-circle below-popup-icon"></i><span class="below-icon-text"> Students Correctly Answered -</span> <span class="below-icon-text">'+iData.numberOfStudentsCorrectlyAnswered+'</span></div>';
			...
			success:function(data){
			  ...
			  var blevel2 = [];
			   var clevel3 = [];
				 ...
				 appenddata+='<table id="subjecttable" class="table table-bordered display pb-30" style="width: 75%; margin-left: auto;margin-right: auto;">'+
												'<thead class="th_background"><tr>'+
								 				' <th style="background:deepskyblue"></th><th>Difficulty Level</th><th>Score</th>'+
...
appenddata+= '<tr>';
				if(j==0){
						appenddata+= '<td rowspan = "'+dificultylevel.length+'" style="background:deepskyblue;color:white;font-weight:bold">'+datajsony+'</td>';
													}
									
														
									appenddata+='<td>'+dificultylevel[j].difficultyLevel+'</td>'+
														'<td style="color:green" data-toggle="modal" data-target="#not-correct-answer-qns" onclick="get_dificulyquestions(\'Scored\',\'' + dificultylevel[j].difficultyLevel+ '\',\'' +subjectlist[i].subjectId+ '\',\'' + testPostId+ '\')">'+dificultylevel[j].scoreMarks+'/'+dificultylevel[j].totalMarks+' <br>('+dificultylevel[j].scoreQuestionsCount+'Q/'+dificultylevel[j].totalQuestionsCount+'Q)</td>'+
														'<td style="color:pink" data-toggle="modal" data-target="#not-correct-answer-qns" onclick="get_dificulyquestions(\'Negative\',\'' + dificultylevel[j].difficultyLevel+ '\',\'' +subjectlist[i].subjectId+ '\',\'' + testPostId+ '\')">'+dificultylevel[j].negativeMarks+'<br> ('+dificultylevel[j].negativeQuestionsCount+'Q)</td>'+
														'<td style="color:blue" data-toggle="modal" data-target="#not-correct-answer-qns" onclick="get_dificulyquestions(\'Unattempted\',\'' + dificultylevel[j].difficultyLevel+ '\',\'' +subjectlist[i].subjectId+ '\',\'' + testPostId+ '\')">'+dificultylevel[j].unAttemptedCount+'</td>'+
														'<td style="color:orange" data-toggle="modal" data-target="#not-correct-answer-qns" onclick="get_dificulyquestions(\'OverTime\',\'' + dificultylevel[j].difficultyLevel+ '\',\'' +subjectlist[i].subjectId+ '\',\'' + testPostId+ '\')">'+dificultylevel[j].overTimeCount+'</td>'+
				...
				clevel3.push(dificultylevel[jk].percentage);
				 ...
				 Highcharts.chart('high', {
                              ...
                              chart: {type: 'column'},
							   title: { text: 'Subject VS difficultyLevel'},
							     exporting: { enabled: false },
                                ...
                                colors:['lightgreen', '#e0b931','#289cd7'],
												series: [{
													name: 'level1',
													data: alevel1
												}, {
...
var inputdata = JSON.stringify(inputData);
	...
	$.ajax({
		...
		success : function(data) {
			...
			appenddata += '<div class="col-md-3" style="border-radius: 20px; margin-right: 18px; width: 180px; height: 120px; padding: 8px 15px; color: black; border: 2px solid deepskyblue">'+
									'<p class="text-center">'+statename.state+'</p>'+
									'<p class="text-center">Rank:<span style="color: lightblue">'+statename.rank+'</span></p>'+
									'<p class="text-center" style="margin-top: 15px">Attempted:'+statename.attempted+'</p></div>';
			var cities = statename.cityRanks;
			for (var i = 0; i < cities.length; i++) {
				
				appenddata += '<div class="col-md-3" style="border-radius: 50%; margin-right: 18px; padding: 8px 15px; color: black; border: 2px solid deepskyblue">'+
									'<p class="text-center">'+cities[i].city+'</p>'+
									'<p class="text-center">Rank:<span style="color: lightblue">'+cities[i].rank+'</span></p>'+
									'<p class="text-center" style="margin-top: 15px">Attempted:'+cities[i].attempted+'</p></div>';
			}
...
var inputdata = JSON.stringify(inputData);
	...
	$.ajax({
		...
		success : function(data) {
			...
			appendhead +='<thead class="th_background" id="overallheader"><tr><th></th><th>Photo</th><th>Student Name</th><th>AIR</th><th>Total Marks</th><th>-ve Marks</th><th>Unattempted</th>';
			...
			appendhead +='<th>Percentage %</th><th>Percentile</th><th>Qns./ Hr.</th><th>Accuracy</th></tr></thead><tbody>';
			for (var i = 0; i < topperdetails.length; i++){
				...
				appendhead +='<td>'+topperdetails[i].percentage+'</td><td>'+topperdetails[i].percentile+'</td><td>'+topperdetails[i].speed+'</td>'+
								'<td>'+topperdetails[i].accuracy+'</td></tr>';
...
function gettoperdetails(index){
	...
	for(var i = 0; i < topperdetails.length; i++){
		if(topperdetails[i].studentImage == ""){
				...
				appenddata += '<td><img src="'+image+'" alt="logo" class="imgstyle-2" style="height: 50px;"></td><td>'+studentname+'</td>'+
				               '<td>'+topperdetails[i].allIndiaRank+'</td><td>'+topperdetails[i].totalMarks+'</td>'+
								'<td>'+topperdetails[i].negativeMarks+'</td><td>'+topperdetails[i].unAttempted+'</td>';
				appenddata +='<td>'+topperdetails[i].percentage+'</td><td>'+topperdetails[i].percentile+'</td><td>'+topperdetails[i].speed+'</td>'+
								'<td>'+topperdetails[i].accuracy+'</td></tr>';
