...
$(document).ready(function() {
	...
	standardId=sessionStorage.getItem("standardId");
	 ...
	 getchap();
	 vType=getParameterByName("allVideo")
	 
	$("#videoType").html(vType);
	studentvideoSub();
...
function displayDays(){
	...
	var allVideos = getParameterByName("allVideo");
						 ...
						 for(var x =0; x<pvideo.length;x++){
							...
							if(studentvideoSub){
								physicsappenddata +='<div style="display:inline-block;"><div class="panel" style="display:inline-block;padding:0px;margin-right:10px;cursor: pointer;border-radius:5px">'+
				 					'<div class="panel-body datavid'+pvideo[x].topicId+'"  data-id='+pvideo[x].topicId+' data-videolink='+pvideo[x].videoLink+' style="padding:0px; marging:0px;" onclick="joinNow1('+"'"+vType+"'"+','+"'"+data[c].chapterId+"'"+',this)">'+
				      			...
				      			'</div>'+*/
								'<div style="background-image:url(' + "'" +img1+ "'" + ');background-repeat: no-repeat;background-size:cover;height:267px;width:393px;border-radius:5px">'+
								...
								physicsappenddata +='<div style="display:inline-block;"><div class="panel" style="display:inline-block;padding:0px;margin-right:10px;cursor: pointer;border-radius:5px">'+
				 					'<div class="panel-body datavid'+pvideo[x].topicId+'"  data-id='+pvideo[x].topicId+' data-videolink='+pvideo[x].videoLink+' style="padding:0px; marging:0px;" data-toggle="modal" data-target="#statusSubscription" onclick="subscriptionPop()">'+
				      			/*'<div  style="background-image:url('+"'"+pvideo[x].tumbNail+"'"+');display:flex;justify-content:center;align-items:center;width:430px;height:180px;">'+
				      			'<i class="fa fal fa-caret-right fa-2x text-white"></i>'+
				      			'</div>'+*/
								'<div style="background-image:url(' + "'" +img1+ "'" + ');background-repeat: no-repeat;background-size:cover;height:267px;width:393px;border-radius:5px">'+
	...
	for(var c =0; c<physicsVideos.length;c++){
		pvideo=physicsVideos[c].videosForUnitWise;
		 for(var x =0; x<pvideo.length;x++){
	 			...
	 			physicsappenddata +='<div class="panel panel-default" style="display:inline-block;padding:0px;margin-right:10px;cursor: pointer;">'+
 			
	 			'<div class="panel-body datavid'+pvideo[x].topicId+'"  data-id='+pvideo[x].topicId+' data-videolink='+pvideo[x].videoLink+' style="padding:0px; marging:0px;" onclick="joinNow1('+"'"+vType+"'"+','+"'"+psub+"'"+',this)">'+
      			'<div  style="background-image:url('+"'"+pvideo[x].tumbNail+"'"+');display:flex;justify-content:center;align-items:center;width:430px;height:180px;">'+
      			'<i class="fa fal fa-caret-right fa-2x text-white"></i>'+
      			'</div>'+
		...
		alertify. alert("No Physics details");
...
function chemistry(){
	...
	for(var c =0; c<chemistryvideos.length;c++){
	 ...
	 for(var y =0; y<cvideo.length;y++){
	 			...
	 			chemistryappenddata +='<div class="panel panel-default" style="display:inline-block;padding:0px;margin-right:10px;cursor: pointer;">'+
	 			'<div class="panel-body datavid'+cvideo[y].topicId+'"  data-id='+cvideo[y].topicId+' data-videolink='+cvideo[y].videoLink+' style="padding:0px; marging:0px;" onclick="joinNow1('+"'"+vType+"'"+','+"'"+csub+"'"+',this)">'+
      			'<div  style="background-image:url('+"'"+cvideo[y].tumbNail+"'"+');display:flex;justify-content:center;align-items:center;width:430px;height:180px;">'+
      			'<i class="fa fal fa-caret-right fa-2x text-white"></i>'+
      			'</div>'+
...
function maths(){
	...
	for(var m =0; m<mathsvideos.length;m++){
	 ...
	 for(var z =0; z<mvideo.length;z++){
	 			...
	 			mathsappenddata +='<div class="panel panel-default" style="display:inline-block;padding:0px;margin-right:10px;cursor: pointer;">'+
	 			'<div class="panel-body datavid'+mvideo[z].topicId+'"  data-id='+mvideo[z].topicId+' data-videolink='+mvideo[z].videoLink+' style="padding:0px; marging:0px;" onclick="joinNow1('+"'"+vType+"'"+','+"'"+msub+"'"+',this)">'+
      			'<div  style="background-image:url('+"'"+mvideo[z].tumbNail+"'"+');display:flex;justify-content:center;align-items:center;width:430px;height:180px;">'+
      			'<i class="fa fal fa-caret-right fa-2x text-white"></i>'+
      			'</div>'+
...
function subscriptionPop(){
			var subscriptionSta='';
			subscriptionSta +='<div class="modal fade" id="statusSubscription" style="background-color:rgba(0,0,0,0.6);" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">'+
    			...
    			'<div class="modal-content" style="border-radius:5px;">'+
					...
					'<div style="padding:20px 0px">'+
      				'<h3 class="text-center text-warning" style="#fd7e14" >For Full Access</h3>'+
