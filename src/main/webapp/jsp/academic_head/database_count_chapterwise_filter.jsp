...
<script src="<%=request.getContextPath()%>/assets/js/academichead.js"></script>
 ...
 $(document).ready(function() {
		...
		var subjectc = getParameterByName("id");
	     var statandard = getParameterByName("std");
	     var levelid =getParameterByName("level");
			var inputData = {
					"subjectId":subjectc,
					"academicheadId":getAHId(),
					"standardId":statandard,
			...
			$.ajax({
				type:"POST",
				  ...
				  url: base_url+'rest/academichead/v1/getchapterwisecountFilter',
				  data:chaptercount,
				  success: function(data){
					 
					  for(var i=0;i<data.length;i++){
					 adappend += "<tr><td>"+data[i].chaptername+"</td>";    
