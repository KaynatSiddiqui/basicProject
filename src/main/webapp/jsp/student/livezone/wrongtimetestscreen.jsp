	...
	<!-- /Main Content -->
		...
		<div class="modal fade" id="questionending" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
	       ...
	       <div class="modal fade" id="questionempty" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
			...
			<div class="modal-content">
			<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" id="wrongclose"  aria-label="Close"><span aria-hidden="true">&times;</span></button>
			</div>
			<div class="modal-body">
			<p class="text-center" style="font-weight:bold;color:black;font-size:17px;">Please select an option</p>
			<button class="btn btn-success btnstyle" id="wrongnooption">Ok</button>
			</div>
			</div>
			</div>
			</div>		
	...
	<script>
	 ...
	 $(document).ready(function(){
		...
		$(document).on("click","#open" ,function(){
			...
			window.location= base_url+"jsp/student/livezone/wrong&timetest.jsp?exam_id="+getParameterByName("examid")+"&subid="+subid;
		})
...
initSeconds: 0
    ...
    var print = function(msg) {
 	...
 	$("#wrongnooption").on("click", function(){
 		$("#wrongclose").click();
 	})
