							...
							<div class="row">
								<div class="col-md-5 col-md-offset-5">
									<div class="button-list mt-10 mb-10">
										<a id="manual_question_back" class="btn btn-back" href="" >Back</a>
...
<script src="<%=request.getContextPath()%>/assets/js/testpost.js"></script>
  	...
  	$(document).ready(function() {
		...
		$.ajax({
			...
			/*  url: base_url+'rest/question/sqbpreviewquestion/'+qestId, */
	...
	if(questionId!=='' && questionId !== null) {
			...
			if(referrer.indexOf("manual_view.jsp") !== -1){
				var teststId = getParameterByName('tid');
				$("#manual_question_back").attr('href',base_url+'jsp/admin/test_post/manual_view.jsp?id='+teststId);
				
			} else if(referrer.indexOf("by_parameters.jsp") !== -1){ 
				$("#manual_question_back").attr('href',base_url+'jsp/admin/database/by_parameters.jsp');
				
			} else if(referrer.indexOf("by_date.jsp") !== -1){ 
				$("#manual_question_back").attr('href',base_url+'jsp/admin/database/by_date.jsp');
				
			} else if(referrer.indexOf("by_previous_year.jsp") !== -1){ 
				$("#manual_question_back").attr('href',base_url+'jsp/admin/database/by_previous_year.jsp');
				
			} else if(referrer.indexOf("difficulty_level.jsp") !== -1){ 
				$("#manual_question_back").attr('href',base_url+'jsp/admin/database/difficulty_level.jsp');
				
			} else if(referrer.indexOf("deleted_questions.jsp") !== -1){ 
				$("#manual_question_back").attr('href',base_url+'jsp/admin/database/deleted_questions.jsp');
				...
				/* getQuestionDataById(questionId,function(wholeQuestionData){ */
					...
					if(intiRegex.test(layoutRes)){
						...
						for(var pq=1;pq<qdata.length;pq++){
							...
							for(var opt=1;opt<=4;opt++){
								appenddata += '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" id="option'+opt+'-div"><div class="row form-group"><div class="col-lg-2 col-md-2 col-sm-3 col-xs-10"><label class="control-label" for="example-email">Option '+opt+'</label></div><div class="col-lg-1 col-md-1 col-sm-1 col-xs-1">:</div><div class="col-lg-9 col-md-9 col-sm-9 col-xs-12" style="color:black" id="option'+opt+'">'+qdata[pq]["option"+opt].value+'</div></div></div>';
							}
				 ...
				 if(sqbQuestionChapterWiseParameters !=="" && sqbQuestionChapterWiseParameters !== null){
					  for(var chap = 0; chap < sqbQuestionChapterWiseParameters.length;chap++)
					 {
						...
						$("#chapterwiseparam").append(cnospaceid+', ');
					 } 
				 }
