...
if(questionId !=='' && questionId !== null) {
	sessionStorage.setItem("admin_editmode", true);	
 ...
 topicappenddata += '<option value="'+data[i].topicId+'">'+data[i].topic+'</option>';
												  ...
												  $.ajax({
													    ...
													    error: function(e){
															...
															//$('.selectpicker .sky_blue_border').selectpicker('refresh');
															  ...
															  sessionStorage.setItem('prevyears_exam',PreviousExamTypeLists[0].competativeExamtypeId);*/
															   var PreviousExamTypeLists = pd.sqbPreviousExamTypeLists;
															 ...
															 if(PreviousExamTypeLists.length !== 0){
															  ...
															  if(chapterparams!== "" && chapterparams !== null){
																  ...
																  for(var cpm = 0; cpm < chapterparams.length;cpm++){
																	  ...
																	  $("#"+cnospaceid).attr("data-rowid",chapterparams[cpm].paramterId);
																  }
															 ...
															 var comment = $("#comments").val(pd.comments);
     ...
     function checkFunction(callback){
			var cf = pd.sqbPreviousExamTypeLists[0].competativeExamtypeId;
    	 var check = $( "#prevyears_exam option:selected" );
  			if(check != cf){
  				return true;
  			}else{
  				return false;
		...
		sessionStorage.setItem("admin_editmode", false);
  ...
  /* 	var check = document.getElementById("prevyears_exam");
  	var checkFunction = function(){
  		if(check != competativeExamtype){
  			return false;
  		}else{
  			return true;
  		}
